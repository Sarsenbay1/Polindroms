...
public class PeremutationWordFinder implements IWordFinder {
    ...
    @Override
    public String permutedWords(String _text) {
        HashMap<String, String> peremutedWords = searchPermutedWords(_text);
        String result = peremutedWords.toString().replaceAll("[{}]", "");
        // System.out.println(result);
        return result;
    }

    protected HashMap<String, String> searchPermutedWords(String _text) {
        // Разбиваем входную строку на слова и сохраняем их в массив _words
        String[] _words = textFormatting(_text);
        ...
        for (Integer i = 0; i < _words.length; i++) {
            for (Integer j = i + 1; j < _words.length; j++) {
                ...
                if (Arrays.equals(_word1, _word2) && !_permutations.containsKey(_words[i])
                        && !_permutations.containsKey(_words[j])
                        && !_permutations.containsValue(_words[i])
                        && !_permutations.containsValue(_words[j])
                        && !_words[i].equals(_words[j])) {
